...
interface GameState {
  ...
  // (swiped-to but not committed). Holds the dog + the FRAGMENT of its zone the
  // beacon lights up: `spot` is the candidate quest point, `radiusM` the patch
  // size. While set, the camera stays tied to the dog (zoomed out) and the fog
  // layer washes that patch brand-blue; NO route is drawn. Tapping commits it
  // (reuses `spot` as the quest target + fetches the route). Free: no API call.
  searchPreview: { dogId: string; spot: LatLng; radiusM: number } | null;
  ...
  // so the lead stops), or pass null to leave preview.
  setSearchPreview: (
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  setSearchPreview: (searchPreview) =>
    set(
      searchPreview
        ? { searchPreview, searchTarget: null, searchRoute: null }
        : { searchPreview: null },
    ),
