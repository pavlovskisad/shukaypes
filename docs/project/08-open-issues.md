...
e-mail and a password before the map opens, and a password login carries
a revocable refresh token rather than the device id. What remains of the
finding is narrower — a device id that leaks still resolves *that* row
until its owner logs in from elsewhere and the row is registered under an
e-mail; and a Telegram user who opens the PWA gets a second row until they
log in by e-mail there (auto-linking `telegram_id` to the e-mail account
is the follow-up, along with the Telegram Login Widget).
