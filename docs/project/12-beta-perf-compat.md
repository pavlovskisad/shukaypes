...
| F-5 | ✅ **Closed 12 Sep, measured on production.** Stored OLX URLs end in `;s=1200x0` and every surface fetched that: 65–122 KB per photo, for a 54 px marker disc. The CDN resizes on request — `;s=240x0` is 16–20 KB, `;s=120x0` 6–7 KB, `;s=480x0` 42–58 KB (600 and 800 were no smaller than 1200 for one pet, so they are not tiers). `utils/petPhoto.ts` now asks for the drawn width × device pixel ratio, snapped to 120/240/480/1200; markers, the card stack and its preload use it. The modal-open preload also went from *every* pet at full size (184 × ~90 KB, up to 20 MB) to the two swipe neighbours at card size. Non-OLX URLs (the Telegram photo proxy, owner uploads) pass through untouched | `app/utils/petPhoto.ts`, `LostDogMarker.tsx`, `LostDogCardStack.tsx`, `MapView.tsx` | Verify: open a pet card, DevTools → Img → the `olxcdn` requests end in `;s=480x0`; markers in `;s=120x0` or `;s=240x0`. |
...
| F-14 | 🔴 **Google Places has been out of daily quota since June** — filed as P1-11. Every spots request fans out up to 175 failing calls before serving three-month-old rows, silently; the parks half runs inside every map sync | `server/src/services/placesCache.ts`, `gameStore.syncMap` | Owner: the daily cap in the Google console. Code: a per-day circuit breaker, stale-beyond-TTL, a client timeout, parks off the sync's critical path. |
| F-15 | ✅ **Closed 14 Sep (D-74).** A GPS fix outside the served area (Lima, sixty km out — Kyiv's air defence, during alarms) is held rather than believed; the server refuses it too. Found while chasing "everything lags with GPS on": the accepted Lima fix put the dog outside the map and the camera fought its own bounds every tick | `app/hooks/useLocation.ts`, `server/src/lib/servedArea.ts` | Verify on the next alarm: the pill «gps глушать — стоїмо тут» appears, the map stays on your street, the HUD stays. |
