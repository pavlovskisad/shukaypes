...
| `lore_favourites` | (user, landmark) pairs — the places a walker hearted. Cascades from both `users` and `kyiv_lore` |
| `kyiv_lore` | Curated geo-indexed landmark stories (OSM + Wikidata, rewritten by Sonnet). `story` is the one-liner; `facts` (jsonb) holds the OSM tags beyond the name — inscription, subject, date; `detail` (2–4 sentences) and the Wikipedia handle behind "read more" are filled by `enrich:lore`, with `wiki_source` saying whether the handle came from the OSM tag, a Wikidata sitelink, the memorial's subject, the row's own name as a title, or a name-matched geosearch. `title` (nullable) is what the object is — "Будинок, де працював Лесь Курбас" — for rows named after a person; the app shows `title ?? name` |
...
- **Bots** (`services/bots.ts`, 571 lines): `MULTIPLAYER_BOTS` (30 in
  ...
  hours (D-77, `botDay.ts`: two or three walks a day, offline between). They write to the same presence
