...
attractions in `kyiv_lore`) merged with whatever Google Places returned, deduped
by position at 120 m because the two sources have unrelated ids for the same
park. Places is no longer required: with its key off, `spots` and `parks` are
empty and the whole pool comes from our own tables. Streets, neighbourhoods,
districts, metro stations and `kyiv_lore`'s 1676 `historic` rows are
deliberately excluded — a street is not a destination and a wall plaque is a
**walk stop**, not somewhere to walk to.
