...
| `AVATAR_RECIPE` | Optional. `marker` (default): the photo alone, style in words. `reference`: the photo beside three of the illustrator's drawings (`server/assets/avatar-refs`, shipped in the image) on the multi-image endpoint — **do not switch it on as is**: on its first real run (13 Sep) the model drew the reference bulldog instead of the owner's retriever. Kept for experiments with another model or reference. |
