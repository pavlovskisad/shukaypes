...
| Happiness index (D-75) | Not an env var: `balance.happinessIndex` — `onlineWindowMs` (90 s; a `/state` poll within it means the person is with the dog: the meter drains and time counts) and `minActiveS` (3600; counted life before a dog ranks). The two totals live on `companion_state` (`happy_weight_s`, `active_s`, `last_poll_at`; migration 0044). Bots' indices are a fiction in `services/happiness.ts`. |
