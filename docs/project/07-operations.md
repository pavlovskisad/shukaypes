...
| `wipe:territory [--all] [--apply] [--snapshot=<path>]` | Wipes the territory layer for a fresh city (D-78): the bots' marks, ground and raids by default, everyone's with `--all`, plus the last-mark spacing and home-ground flag on the affected companion rows. Dry by default and prints what is held and exactly what the apply deletes; `--apply` does it, not reversible, `--snapshot` writes the rows to JSON first. Progression stays. Production: `fly ssh console -a shukajpes-api -C "node dist/db/wipe-territory.js"`, read the dry run, then again with `--apply`. |
