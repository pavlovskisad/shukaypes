...
| `bots:avatars [i…]` | Draws the thirty bots' portraits (D-73) from the hand-written roster in `services/botAvatars.ts` through the D-72 recipe and the ink pass into `server/assets/bot-avatars/<i>.png`; only the missing ones unless indices are given. Needs `FAL_KEY`; paid calls, about a dollar for all thirty; the files are committed like any asset. Touches no table. |
