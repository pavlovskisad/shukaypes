...
| `AVATAR_RECIPE` | Optional. `describe` (default): Claude (Haiku, the ambient model) looks at the photo once and writes one sentence for a caricaturist; Nano Banana (via fal) then draws from that sentence and eight of the illustrator's drawings, with no photo in the request. `reference`: the photo goes to Nano Banana beside the drawings — the automatic fallback when the description cannot be made. `marker`: the photo alone on FLUX Kontext, style in words — the fallback when the reference files are missing. Flip with `fly secrets set` to compare on a real pet. |
