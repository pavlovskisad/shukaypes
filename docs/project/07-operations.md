...
| `AVATAR_RECIPE` | Optional. `describe` (default): Claude (Haiku, the ambient model) looks at the photo once and writes one labelled line for a caricaturist — coat tone and texture, ears, muzzle, markings, look; Nano Banana (via fal) then draws from that line and eight of the illustrator's drawings, with the photo last as a likeness check only. The line is logged (`avatar_describe`); it is about an animal's coat and nothing else. `reference`: the photo goes to Nano Banana beside the drawings with no description — the automatic fallback when the description cannot be made. `marker`: the photo alone on FLUX Kontext, style in words — the fallback when the reference files are missing. Flip with `fly secrets set` to compare on a real pet. |
...
| `AVATAR_INK` | Optional. `off` skips the app's own ink pass over the model's drawing (`services/ink.ts`: pure black and white, thickened to marker weight, wobbled, and warped by one slow wave so the head is lopsided). On by default; the pass is where the fat uneven line and the wonky proportions come from. |
