...
| `AVATAR_INK` | Optional. `off` skips the app's own ink pass over the model's drawing (`services/ink.ts`: pure black and white, thickened to marker weight, wobbled, small loose blobs erased, cropped so the pet fills the frame). On by default; the pass is where the fat uneven line comes from. |
