...
| `AVATAR_INK` | Optional. `off` skips the app's own ink pass over the model's drawing (`services/ink.ts`: pure black and white, thickened to marker weight, wobbled). On by default; the pass is where the fat uneven line comes from. |
| `AVATAR_DAILY_CAP` | Optional. Drawings per person per day, default 5. Raise it while tuning on a real phone; each drawing is a paid call. |
