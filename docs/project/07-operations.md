...
| `AVATAR_RECIPE` | Optional. `reference` (default): the photo goes to the model beside three of the illustrator's drawings (`server/assets/avatar-refs`, shipped in the image) on the multi-image endpoint. `marker`: the photo alone, style in words — the cheaper call and the automatic fallback when the reference files are missing. Flip it with `fly secrets set` to compare on a real pet. |
