...
| `AVATAR_DAILY_CAP` | Optional. Drawings per person per day, default 100 — a guard against a script, not a person; each drawing is a paid call of a few cents. Lower it if the bill ever says so. |
| `BOT_AVATARS_DIR` | Optional. Where the bots' drawn portraits live (D-73), default `server/assets/bot-avatars`; `<i>.png` per `bot:<i>`, served at `/bot-avatars/<i>.png`. A missing file is a chip with the dog sprite, not an error. The chip-or-sprite switch for the map is `OTHER_WALKER_STYLE` in `app/constants/experiments.ts`, not an env var. |
| Happiness index (D-75) | Not an env var: `balance.happinessIndex` — `onlineWindowMs` (90 s; a `/state` poll within it means the person is with the dog: the meter drains and time counts) and `minActiveS` (600; ten minutes of counted life before a dog ranks). The two totals live on `companion_state` (`happy_weight_s`, `active_s`, `last_poll_at`; migration 0044). Bots rank on the same terms since D-76. |
| `MULTIPLAYER_BOTS` | The number of bot walkers (0 = off); the roster and portraits cover 120 (D-78), which on an owner's hours (D-77) shows 12–15 dogs in the evening and ~21 at the morning peak. Since D-76 they live by the player's rules: a companion row each, online sessions that count toward the happiness index and drain happiness, food and paws spawned and picked up through the same code a phone uses, marks that cost and are refused the same. The cron logs `mp_bot_life` every five minutes (bones, paws, marks, refusals); `collect_events` is the per-bot ledger. |
| `WALK_MAX_SPEED_MPS`, `TERRITORY_COOLDOWN_MS` | Already the dials for the walk gate and the mark spacing; named here because they are also what a **scripted** walk needs. A harness posting `/collect/path` fires its syncs milliseconds apart, so a 600 m step reads as thousands of m/s and the speed gate refuses it — correctly — and every mark after the first hits the 20 s cooldown. Raise the first and drop the second to drive a walk from a script; leave both alone anywhere a real phone is talking. |
| `MULTIPLAYER_BOTS_ALWAYS_ON` | `1` keeps every bot on the map around the clock. Unset (production), each bot keeps an owner's hours (D-77, `services/botDay.ts`): two or three walks a day of 20–45 minutes on the Kyiv clock, offline in between — about 5% online, two to five of thirty out at a time. Set it on a local stack so a test at night still finds dogs. `pnpm check:bot-day` prints the timetable's measured share. |
| `AVATAR_CHAT_ID` | Optional. The Telegram chat the bot uploads drawings to for keeping (their `file_id` is the storage). Unset, drawings go to `ALERT_CHAT_ID`, captioned with the nickname and the pet's name — a moderation feed for free. Neither set with `FAL_KEY` set: drawings fail with `avatar_unstored`. |
...
| `/admin/accounts[?format=text]` | `DASHBOARD_TOKEN` or admin | The people (D-85): registered split from anonymous device rows, plus per-account nickname, pet, verification, avatar, Telegram, points, distance and timestamps. **The only admin endpoint carrying personal data** — addresses are masked (`sa•••@gmail.com`) by `lib/maskEmail` before they leave the process, so the full string is never in the payload. |
...
**Unset, and tuning dials rather than secrets** (D-87): `WALK_MAX_SPEED_MPS`
...
| `probe:avatar [recipe] [pet]` | Draws the three pets in `server/assets/avatar-probe` with the portrait recipes (Kontext words-only, Nano Banana with one and with three of the illustrator's drawings) into `server/probe-out/` for a human to compare. Needs `FAL_KEY`; paid calls, under a dollar for all nine; touches nothing in the app. |
| `wipe:territory [--all] [--progress] [--apply] [--snapshot=<path>]` | Wipes the territory layer for a fresh city (D-78): the bots' marks, ground and raids by default, everyone's with `--all`, plus the last-mark spacing and home-ground flag on the affected companion rows. `--progress` (D-79) also zeroes the happiness index (`happy_weight_s`, `active_s`) and resets the meters, for when the counted hours were measured under a rhythm that no longer applies; XP, level and points stay (`wipe:stats` is for those). Dry by default and prints what is held and exactly what the apply deletes; `--apply` does it, not reversible, `--snapshot` writes the rows to JSON first. Progression stays. Production: `fly ssh console -a shukajpes-api -C "node dist/db/wipe-territory.js"`, read the dry run, then again with `--apply`. |
| *(territory panel, D-89)* | No command — it is a panel on `/admin/console` and a block in `/admin/metrics`. `unclaimed marks` is the one to read: marks that can never form a hull (fewer than `shapeMinMarks` of the same owner within `claimNeighbourM`). High share → a white patch on the map is the shape rule biting, not missing walkers; low share → the ground really is unwalked. The two want opposite fixes. |
| `check:catch-up` | Fixture check for `services/catchUp` (D-87): the speed gate lets a walk through and refuses a bike and a car, a foreground segment is never judged on speed, and every catch-up plan clears **both** spacing rules the live mechanic enforces — metres and seconds — across ~11,000 generated cases. Guards the invariant that a pocketed walk can never claim faster or denser than a walk with the screen on. No network, no database. Part of the `pnpm check` aggregate. |
| *(app-side checks, `pnpm --filter app check`)* | The client's two fixture checks, run by the root `pnpm check` alongside the server's aggregate. `utils/pinLifecycle.check.ts` (D-91) replays a tap from the standing through the camera flight, guarding that a pinned dog survives a stale viewport and is let go only on going away or leaving the map. `utils/doorScreen.check.ts` (D-96) replays the gate's two answers, guarding that the person's answer names the screen and only a held reset token or an unverified account may override it — the rule whose absence made registration unreachable after a logout. `utils/cameraHold.check.ts` (D-98) walks the `ReadingSurfaces` type and guards that every surface the walker can read on the map holds the camera on its own — the follow and the hint snap share that one list, and the two hand-written lists it replaces had drifted. All three are mutation-tested; no network, no database, no React. |
| `check:daily` | Fixture check for `services/dailyRewards` (D-99): the day's six pay once each at their target and never again as the counter climbs past it, the bonus lands with the task that completes the set and not before, land crosses in metres rather than counts, and a retired task key left in `paid` is ignored. Guards the arithmetic behind the only thing in the game that pays a currency the walker can see. Mutation-tested; no network, no database. Part of the `pnpm check` aggregate. |
| `check:mask` | Fixture check for `lib/maskEmail` (D-85): two characters revealed, exactly three bullets, the domain kept, nothing thrown on junk, and the address length never recoverable from the mask. Guards a privacy property — the accounts panel's addresses are masked server-side. No network, no database. |
| `bots:avatars [i…]` | Draws the roster's portraits (D-73; 120 entries since D-78, was thirty) from the hand-written roster in `services/botAvatars.ts` through the D-72 recipe and the ink pass into `server/assets/bot-avatars/<i>.png`; only the missing ones unless indices are given. Needs `FAL_KEY`; paid calls, about a dollar for all thirty; the files are committed like any asset. Touches no table. |
...
| `check` (`pnpm check`) | All **twenty-three** fixture checks — placement judge, placement confidence, lore walk, lore match, enrich parse, out-of-area, ingest alert, pet identity, per-user rate limiting, invite gate, dev auth, session token, served area, contact redaction, ad-body containment, ad extraction, found reports, owner reports, place resolution, route coverage, accounts, ink, catch-up. **Runs in CI on PRs and before deploy.** `check:bot-day` and `check:mask` are deliberately outside the aggregate — run them by name when touching the bots' timetable or the address mask. |
