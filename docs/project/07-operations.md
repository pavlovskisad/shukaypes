...
| `AVATAR_RECIPE` | Optional. `reference` (default): the photo goes to Nano Banana (Gemini image edit, via fal) beside four of the illustrator's scribbliest drawings (`REF_FILES` in `services/avatar.ts`, files in `server/assets/avatar-refs`, shipped in the image). `marker`: the photo alone on FLUX Kontext, style in words — a good likeness in the wrong hand (13 Sep), and the automatic fallback when the reference files are missing. Flip with `fly secrets set` to compare on a real pet. |
